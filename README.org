...
structure (specified in `workspace-regex` map) will be automatically
registered or dergesitered from the geoserver. See `file-watcher`
entry in [[file:resources/sample-config.edn][resources/sample-config.edn]] for an example. 

Here's an explanation of it's confgiuration:
- dir: A readable directory
- workspace-regex: a map of folder names to the regexes for parsing
                   the workspace from the file path.
