...
Inorder for imagemosic to work properly three files are needed in these folders
(datastore.properties, indexer.properties, and timeregex.properties). These are
the minimum properties required prior to registration.
...
Note that datatore.properties also need a schema property, but this will be
inserted by geosync based on the cli config or, if in server-mode, the request.
