...
>>>>>>> 7e07dd3 (1. remove `style-exists?` function)
...
It is possible to sync styles to GeoServer using the `style-dir`
key in your `.edn` file.

When you configure a `style-dir` (`-s` or `--style-dir` option in the CLI),
GeoSync will look up all `.css` files recursively in this directory and it
will add them to GeoServer for you.

This step happens before any layers are registered, so you can mix and match
the styles configuration with already available options on GeoSync.

There is a parameter called `overwrite-styles` (available in the CLI as `-O` or `--overwrite-styles`)
that will control how GeoSync processes the styles. If enabled, any already existing style
will be updated. Defaults to `nil` and will be considered `false`.

The styles are tied to your workspace, so if you remove your workspace, the styles will
also be removed.
